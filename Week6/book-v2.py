...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            # 🧩 Giải mã phần payload mà không verify để lấy username trước
            unverified_payload = jwt.decode(token, options={"verify_signature": False})
            username = unverified_payload['user']
...
def login():
    ...
    if username == 'admin' and password == '123456':
        ...
        # 3️⃣ Lưu public key vào MySQL
        existing_key = KeyToken.query.filter_by(user_id=username).first()
        if existing_key:
            db.session.delete(existing_key)
        new_key = KeyToken(user_id=username, public_key=public_pem.decode('utf-8'))
        db.session.add(new_key)
        db.session.commit()

        # 4️⃣ Tạo JWT với private key
        payload = {
            'user': username,
        ...
        return success_response({"token": token}, "Login successful (RSA)")
