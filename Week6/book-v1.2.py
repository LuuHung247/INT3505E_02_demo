...
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY')  # Dùng để mã hóa JWT
...
def login():
    ...
    if username == 'admin' and password == '123456':
        ...
        # Tạo và lưu refresh token (7 ngày)
        rt = create_and_store_refresh_token(username)

        # Gửi token trong cookie HttpOnly
        ...
        resp.set_cookie(
            ...
            access_token,
            httponly=True,
            secure=False,      # để test local thì False; khi deploy phải True (HTTPS)
            samesite='Strict',
...
def refresh():
    """
    Endpoint dùng để đổi refresh token lấy access token mới.
    Flow:
    - Lấy refresh_token từ cookie
    - Kiểm tra tồn tại & chưa revoked & chưa expired
    - Revoke token cũ (rotation), tạo refresh token mới và access token mới
    - Trả cookie access_token + refresh_token mới
    """
...
def logout():
    """
    Logout: revoke refresh token (nếu có) và clear cookie access + refresh
    """
    refresh_token_value = request.cookies.get('refresh_token')
    ...
    # Clear cookies (set expires in past)
