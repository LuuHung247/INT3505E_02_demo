from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from openapi_server.models.base_model import Model
from openapi_server import util


class Member(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, id=None, name=None, email=None, join_date=None):  # noqa: E501
        """Member - a model defined in OpenAPI

        :param id: The id of this Member.  # noqa: E501
        :type id: int
        :param name: The name of this Member.  # noqa: E501
        :type name: str
        :param email: The email of this Member.  # noqa: E501
        :type email: str
        :param join_date: The join_date of this Member.  # noqa: E501
        :type join_date: str
        """
        self.openapi_types = {
            'id': int,
            'name': str,
            'email': str,
            'join_date': str
        }

        self.attribute_map = {
            'id': 'id',
            'name': 'name',
            'email': 'email',
            'join_date': 'join_date'
        }

        self._id = id
        self._name = name
        self._email = email
        self._join_date = join_date

    @classmethod
    def from_dict(cls, dikt) -> 'Member':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The Member of this Member.  # noqa: E501
        :rtype: Member
        """
        return util.deserialize_model(dikt, cls)

    @property
    def id(self) -> int:
        """Gets the id of this Member.


        :return: The id of this Member.
        :rtype: int
        """
        return self._id

    @id.setter
    def id(self, id: int):
        """Sets the id of this Member.


        :param id: The id of this Member.
        :type id: int
        """

        self._id = id

    @property
    def name(self) -> str:
        """Gets the name of this Member.


        :return: The name of this Member.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name: str):
        """Sets the name of this Member.


        :param name: The name of this Member.
        :type name: str
        """

        self._name = name

    @property
    def email(self) -> str:
        """Gets the email of this Member.


        :return: The email of this Member.
        :rtype: str
        """
        return self._email

    @email.setter
    def email(self, email: str):
        """Sets the email of this Member.


        :param email: The email of this Member.
        :type email: str
        """

        self._email = email

    @property
    def join_date(self) -> str:
        """Gets the join_date of this Member.


        :return: The join_date of this Member.
        :rtype: str
        """
        return self._join_date

    @join_date.setter
    def join_date(self, join_date: str):
        """Sets the join_date of this Member.


        :param join_date: The join_date of this Member.
        :type join_date: str
        """

        self._join_date = join_date
